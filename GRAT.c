...
// multithreading default parameters
#define THREADS_NUM 2 // number of (side) threads
#define THREAD_SLEEP_TIME 50 // frequency (in milliseconds) of buffer sampling
#define THREAD_BUFFER_SIZE 1 // for 1 side thread

// the main parametrs that need to be selected for the considered curve
// selected as universal for most curves
#define HASH_SIZE 64
...
void run_thread(DivAndTrack& distD, int b, RWData &rwdata, ZZ p)
{
	ZZ_p::init(p);
	...
	while (!rwdata.killThreads)
	{
		...
		while (!IsZero(distD.D) && !rwdata.killThreads)
...
int main(int argc, char **argv)
{
    ...
    // initialize threads
    unsigned short int thIter = 0;
    thread th[THREADS_NUM];
    int thWalkType[THREADS_NUM];
    DivAndTrack thDistP[THREADS_NUM*THREAD_BUFFER_SIZE];
    rwdata.killThreads = false;
    
	for (int i = 0; i < THREADS_NUM; ++i)
    {
        clear(thDistP[i].D);
        thWalkType[i] = i%2;
        // th[i] = thread(run_thread, ref(thDistP[i]), thWalkType[i], ref(rwdata), p);
    }
	...
	// start detection aa collison
	int b = (RandomWord() & 1UL);
	int found = 0;
	while (!found && !rwdata.killThreads)
	{
        // if a side thread has found a distinguished point, 
        // then it will write the point to the buffer,
        // so the buffer will be non-zero
        if (!IsZero(thDistP[thIter%(THREADS_NUM*THREAD_BUFFER_SIZE)].D))
        {
            distP = thDistP[thIter%(THREADS_NUM*THREAD_BUFFER_SIZE)];
            b = thWalkType[thIter%THREADS_NUM]; // {0: tame, 1: wild}

            // set the value to zero so that the thread continues searching
            clear(thDistP[thIter%(THREADS_NUM*THREAD_BUFFER_SIZE)].D);
    ...
    // send a stop signal to side threads
    rwdata.killThreads = true;

    // and attach them to the main
    for (int i = 0; i < THREADS_NUM; ++i)
        th[i].join();
